<?php
//http://localhost/red_api/index.php?idJob=_auditar_valid_item&token=123&documento=28485068&idPedido=1479&item=330101&cantidad=1&idDelegacion=7

//coseguro
//http://localhost/red_api/index.php?idJob=_auditar_valid_item&token=123&idPedido=1890589&item=330101&cantidad=6

//http://localhost/red_api/index.php?idJob=_auditar_valid_item&token=123&idPedido=189058&item=330101&cantidad=6&documento=1&idUsuario=19
...
$cobrar_coseguro = kselectone("select coseguro from usuarios where idUsuario=" . $delegacion_idUsuario);
...
if ($cobrar_coseguro == "si") {
    $idvalor = $arr[0]['idvalor'];
    $coseguro = $arr[0]['cs02'] * $cantidad;
    $coseguro_unitario = $arr[0]['cs02'];
} else {
    $idvalor = 0;
    $coseguro = 0;
    $coseguro_unitario = 0;
...
switch ($item) {
    case '330101':
        ...
        if ($acumulado_ano >= 30) {
            $response["data1"] = "error";
            $response["data2"] = "ATENCION. Se permiten 30 al año de este item y ya acumula $acumulado_ano";
        } elseif ($acumulado_mes >= 4) {        //ano
            $response["data1"] = "error";
            $response["data2"] = "ATENCION. Se permiten 4 al mes de este item y ya acumula $acumulado_mes";
...
$arr = array(
    ...
    'coseguro'          => $coseguro,           
    'coseguro_unitario' => $coseguro_unitario,
    'idvalor'           => $idvalor,
